    ...
    private static void startGame(final Map<String, String> positions) {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            ...
            for (int j = 0; j < BOARD_LIMIT; j++) {
                final var positionConfig = positions.get("%s,%s".formatted(i, j));
                if (nonNull(positionConfig)) {
                    final var config = positionConfig.split(",");
                    final var expected = Integer.parseInt(config[0]);
                    final var fixed = Boolean.parseBoolean(config[1]);
